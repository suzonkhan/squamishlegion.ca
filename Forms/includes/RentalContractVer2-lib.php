...
class FM_DateObj
{
    ...
    public function __construct(&$formvars,&$config,&$logger)
    {
    ...
    function GetDateFieldInStdForm($var_name)
    {
        if(!isset($this->formvars[$var_name]))
        {
            $this->logger->LogError("date field $var_name is not set. returning false...".@print_r($this->formvars,true));
            return false;
        }
        
        $date_value = $this->formvars[$var_name];

        //Note 14 July 2012: 
        //Date format is always YYYY-MM-DD at server side

        return $this->GetDateFieldInStdFormInternal($date_value,'YYYY-MM-DD');
    }

    function GetDateFieldInStdFormInternal($date_value,$date_format)
    {
        $day=0;
        $mon =0;
        $year=0;
        ...
        $this->logger->LogInfo("GetDateFieldInStdFormInternal:$date_value format:$date_format");
        $date_parts=array();

        if(preg_match('#^(\d*)[\-\\\/\.](\d*)[\-\\\/\.](\d*)$#',$date_value,$date_parts))
        {
            switch($date_format)
            {
                case "DD-MM-YYYY":
        ...
        {
            $monthnames=array("jan"=>1,"feb"=>2,"mar"=>3,"apr"=>4,"may"=>5,"jun"=>6,"jul"=>7,"aug"=>8,
            ...
            switch($date_format)
            {
                case "DD-MON-YYYY":
                   $date_parts=array();
                   preg_match('#(\d*)[\-\\\/\.](\w*)[\-\\\/\.](\d*)#',$date_value,$date_parts);
                   $day = $date_parts[1];
                   $mon = $monthnames[strtolower($date_parts[2])];
                   $year = $date_parts[3];
                   break;

                case "MON-DD-YYYY":
                   $date_parts=array();
                   preg_match('#(\w*)[\-\\\/\.](\d*)[\-\\\/\.](\d*)#',$date_value,$date_parts);
                   $mon = $monthnames[strtolower($date_parts[1])];
                   $day = $date_parts[2];
                   $year = $date_parts[3];
                   break;                           
            }
        ...
        if($year<=100 || $year > 3000 || $mon < 1 || $mon>12 || $day<1 || $day > 31 )
        {
            $this->logger->LogInfo("Received bad date: $date_value");    
            return false;
        }
        else
        {
             $ret_date = sprintf("%04d-%02d-%02d", $year, $mon, $day);
        }
        
        $this->logger->LogInfo("return date in std form: $ret_date");

        return $ret_date;
    }

    function GetOtherDate($command_value)
    {
        $date_ret='';
        if(strcmp($command_value,"SameDay")==0)
        {
            $date_ret = date("Y-m-d");
        }
        else
        {   
            $cmd_parts=array();
            preg_match('#^(\w+)\(([\d\w\-]+)\)$#',$command_value,$cmd_parts);

            if('FixedDate' == $cmd_parts[1])
            {
                $date_ret = $this->GetDateFieldInStdFormInternal($cmd_parts[2],'YYYY-MM-DD');
            }
            else
            if('Field' == $cmd_parts[1])
            {
                $date_ret = $this->GetDateFieldInStdForm($cmd_parts[2]);
            }
        }
        return $date_ret;
    }
...
{
    private $formdata_replacement_variable;
    private $confirm_submit_variable_name;
    private $templ_page;
    private $uploader;
    private $confirm_edit_button_code;
    private $extra_code;
    private $extra_code_place;
   ...
   public function __construct($templ="")
    {
        ...
        $this->extra_code_place='<!--{ExtraCodePlace}-->';
   ...
   function MakeCompleteFormCode($button_code,$session_var_name)
    {
        $action = htmlentities($this->globaldata->get_php_self());

        $str_ret = "<form class='sfm_confirm' action='$action' method='post'>";
        $str_ret .= CreateHiddenInput($this->config->form_submit_variable,"yes");
        $str_ret .= CreateHiddenInput($this->globaldata->saved_data_varname,$session_var_name);
...
class FM_AutoResponseSender extends FM_ComposedMailSender
{
	...
	public function __construct($subj="",$templ="")
	{
        parent::__construct();
        $this->subject = $subj;
        $this->template= $templ;
	...
	{
        $name_val = '';
        if(!empty($this->formvars[$this->namevar]))
        {
            $name_val = $this->formvars[$this->namevar];
        }
...
class FM_Logger
{
   ...
   function get_log()
   {
    ...
    {
        if(!$this->log_file_path || strlen($this->log_file_path)<=0)
        {
    ...
    function writeToFile($t_log)
    {
        ...
        if(!$this->log_file_path || strlen($this->log_file_path)<=0){ return false;}
...
class Crypt_Blowfish_PHP extends Crypt_Blowfish
{
    ...
    {
        //PJ: fix for PHP warning can't pass null to strlen
        if(is_null($iv))
        {
            $this->_iv = '';
        }else{
